...
const BlueOnBlack: SongData = {
  ...
  verseRows: [
    ...
    },
    {
      ...
      lyrics: [
        { text: 'You turned', x: 52 }, { text: 'and you ran,', x: 198 }, { text: 'oh yeah', x: 378 },
      ],
    },
    {
      chords: [
  ...
  chorusRows: [
    {
      chords: [{ name: 'D', x: 52 }, { name: 'C', x: 270 }, { name: 'G', x: 446 }],
      lyrics: [
  ...
  harmonicCols: [
    { title: 'Verse / Intro', chords: 'D – Cadd9 – G', mode: 'D mixolydian', modeDetail: 'D pent. minor',
      row5: 'Safe: D E F# G A C', row6: 'Avoid: C# (diatonic major)', pageRef: '→ p.2 for fretboard map' },
    { title: 'Chorus', chords: 'D – C – G', mode: 'Same palette', modeDetail: 'denser rhythm feel',
      row5: 'C natural = ♭7', row6: 'Strong mixolydian colour' },
    { title: 'Chorus pivot', chords: 'A – G  (bar 3)', mode: 'V chord tension', modeDetail: 'resolves back to D',
      row5: 'A = V of D (dominant)', row6: 'Creates lift before resolve' },
    { title: 'Solo', chords: 'D pent. minor', mode: 'D F G A C', modeDetail: 'no E, no B',
      row5: '⚠ not diatonic minor', row5Warning: true, row6: 'F# clashes here', pageRef: '→ solo tab on p.2' },
  ],
};
...
const App = () => {
  ...
  const handleGenerate = async (title: string, artist: string) => {
    setLoading(true);
    setError(null);
    try {
  ...
  return (
    <div style={{ background: '#222', minHeight: '100vh', padding: '20px' }}>
      <SongSearch onGenerate={handleGenerate} loading={loading} error={error} />
      <div style={{ marginTop: '20px', display: 'inline-block' }}>
        {loading ? (
          <div style={{ color: '#aaa', padding: '40px', fontFamily: 'Georgia, serif' }}>
          ...
          <MapleCanvas>
            ...
            <BandVerse annotation={song.verseAnnotation} annotationX={song.verseAnnotationX} rows={song.verseRows} />
            <BandChorus annotation={song.chorusAnnotation} annotationX={song.chorusAnnotationX} rows={song.chorusRows} />
