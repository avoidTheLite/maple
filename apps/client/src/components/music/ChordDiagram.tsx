...
const ChordDiagram = ({ chord, width = 38, height = 46 }: ChordDiagramProps) => {
  const midX = width / 2;
  const right = width - 4;
  ...
  return (
    <g>
      ...
      {chord.frets.map((fret, i) => {
        const cx = STRING_X[i];
        if (fret === -1) return <text key={i} x={cx} y="5" textAnchor="middle" className="csub">x</text>;
        if (fret === 0)  return <text key={i} x={cx} y="5" textAnchor="middle" className="csub">o</text>;
